class DistrictsController < ApplicationController
  ...
  # POST /districts.xml
  def create
    @district = District.new(params[:districts])

    respond_to do |format|
      if @district.save
        flash[:notice] = 'Districts was successfully created.'
  ...
  def update
    ...
    respond_to do |format|
      if @district.update_attributes(params[:districts])
        flash[:notice] = 'Districts was successfully updated.'
        format.html { redirect_to(@district) }
        format.xml  { head :ok }
      else
